...
class Zone:
    def __init__(self, adapi, name, config={}):
        ...
        self.fingerbot_switch_reverse = self.adapi.get_entity(config["switch_reverse_entity_id"]) if "switch_reverse_entity_id" in config else None
    ...
    async def get_current_temperature(self):
        return float(await self.temperature_sensor.get_state()) if self.temperature_sensor else None
    ...
    async def revert_switch_state_without_clicking(self):
        if self.fingerbot_switch_reverse:
            ...
            await self.fingerbot_switch.set_state(new_state)

    async def toggle_switch_and_wait_state(self, expected_state: str, timeout: int=10):
